...
const RecordSaleForm: React.FC<{ onSaleRecorded: () => void }> = ({ onSaleRecorded }) => {
    ...
    useEffect(() => {
        ...
        const fetchItems = async () => {
            try {
                ...
                const { data } = await api.get('/items?page=1&limit=1000', config); // A bit of a hack to get all items
                setItems(data.items);
            } catch (error) {
    ...
    const selectedItem = items.find(i => i._id === selectedItemId);
    const profitPreview = selectedItem ? (actualSellingPrice - selectedItem.buyingPrice) * quantitySold : 0;
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select value={selectedItemId} onChange={handleItemChange} required className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                            <option value="">Select an item</option>
                            {items.map(item => <option key={item._id} value={item._id}>{item.name} (In Stock: {item.quantity})</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium">Quantity Sold</label>
                        <input type="number" min="1" value={quantitySold} onChange={e => setQuantitySold(Number(e.target.value))} required className="mt-1 block w-full rounded-md border-slate-300 shadow-sm"/>
                    </div>
                     <div>
                        <label className="block text-sm font-medium">Actual Selling Price (per item)</label>
                        <input type="number" value={actualSellingPrice} onChange={e => setActualSellingPrice(Number(e.target.value))} required className="mt-1 block w-full rounded-md border-slate-300 shadow-sm"/>
                    </div>
                     <div>
                        <label className="block text-sm font-medium">Payment Method</label>
                        <select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value as 'Cash' | 'Mpesa')} required className="mt-1 block w-full rounded-md border-slate-300 shadow-sm">
                            <option>Cash</option>
                            <option>Mpesa</option>
                        </select>
                    </div>
                     <div>
                        ...
                        <input type="text" value={attendant} onChange={e => setAttendant(e.target.value)} required className="mt-1 block w-full rounded-md border-slate-300 shadow-sm"/>
                    </div>
                    <div className="flex items-end">
                        <button type="submit" disabled={loading || !selectedItemId} className="w-full px-4 py-2 bg-primary-600 text-white rounded-md shadow-sm hover:bg-primary-700 disabled:bg-primary-300">
                 ...
                 {selectedItemId && (
                    <div className="bg-slate-50 p-3 rounded-md text-sm grid grid-cols-2">
                        <p>Total Sale: <span className="font-bold">KES {(actualSellingPrice * quantitySold).toLocaleString()}</span></p>
                        <p>Est. Profit: <span className={`font-bold ${profitPreview >= 0 ? 'text-green-600' : 'text-red-600'}`}>KES {profitPreview.toLocaleString()}</span></p>
                    </div>
                )}
            </form>
        </div>
                    ...
                    <table className="w-full text-sm text-left text-slate-500">
                        <thead className="text-xs text-slate-700 uppercase bg-slate-50">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3">Total Sale</th>
                         ...
                         <tbody>
                            {loading ? (
                                <tr><td colSpan={7} className="text-center py-8">Loading sales...</td></tr>
                            ) : sales.length === 0 ? (
                                ...
                                sales.map(sale => (
                                    <tr key={sale._id} className="bg-white border-b hover:bg-slate-50">
                                        ...
                                        <td className={`px-6 py-4 font-bold ${sale.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>KES {sale.profit.toLocaleString()}</td>
